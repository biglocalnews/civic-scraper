...
class GranicusType2Scraper(GranicusBaseScraper):
    ...
    def _extract_meeting_details_internal(
        ...
        if not main_tabbed_panels_content_group:
            logger.info(
                f"{self.__class__.__name__}: No 'TabbedPanelsContentGroup' (for years) found. This might not be a Type 2 structure, or the page structure is simpler."
            )
            ...
            for cp_panel in all_collapsible_panels:
                ...
                if header_div:
                    ...
                    if current_panel_name_text == panel_name:
                        ...
                        panel_content_div = cp_panel.find(
                        ...
                        if panel_content_div:
                            ...
                            if panel_content_div.find("div", class_="TabbedPanels"):
                                logger.info(
                                    f"{self.__class__.__name__}: Panel '{panel_name}' content (in fallback) contains an inner 'TabbedPanels' div. This might be Type 1. Skipping for Type 2."
                                )
                                return []
                            meetings.extend(
                                self._extract_meetings_from_panel_content(
                                    panel_content_div, "DefaultYear"
                                )
                            )
                        break
            if not panel_found_in_current_view:
                logger.warning(
                    f"{self.__class__.__name__}: Panel '{panel_name}' not found in current view (fallback)."
                )
            return meetings
        ...
        )  # Assuming it's a sibling or parent of ContentGroup
        ...
        for i, year_content_div in enumerate(year_content_divs):
            ...
            collapsible_panels_in_year = year_content_div.find_all(
            ...
            for cp_panel in collapsible_panels_in_year:
                ...
                header_div = cp_panel.find(
                    ["div", "h2", "h3"],
                    class_=["CollapsiblePanelTab", "CollapsiblePanelTabNotSelected"],
                )  # Common tags for panel headers
                if not header_div:
                    continue

                text_container = header_div.find(
                    ["a", "h3", "span", "div"]
                )  # More specific text elements
                current_panel_name_text = (
                    text_container.get_text(strip=True)
                    if text_container
                    else header_div.get_text(strip=True)
                ...
                if current_panel_name_text == panel_name:
                    ...
                    if panel_content_div:
                        ...
                        if panel_content_div.find("div", class_="TabbedPanels"):
                            logger.info(
                                f"{self.__class__.__name__}: Panel '{panel_name}' (Year: {year_context}) content contains an inner 'TabbedPanels' div. This specific panel might be Type 1. Skipping for Type 2."
                            )
                            continue  # Skip this panel, might be Type 1

                        meetings.extend(
                            self._extract_meetings_from_panel_content(
                                panel_content_div, year_context
                            )
                        )
                    else:
                        logger.warning(
                            f"{self.__class__.__name__}: Panel '{panel_name}' (Year: {year_context}) found, but its 'CollapsiblePanelContent' is missing."
                        )
                    break  # Found the target panel for this year, move to next year

            if not found_panel_in_this_year:
                logger.info(
                    f"{self.__class__.__name__}: Panel '{panel_name}' not found within year '{year_context}' content."
                )

        if not panel_ever_found:
            logger.warning(
                f"{self.__class__.__name__}: Panel '{panel_name}' was not found in any year's content."
            )
    ...
    def _extract_meetings_from_panel_content(
        self, panel_content_div: BeautifulSoup, year_context: str
    ) -> list[dict]:
        """
        ...
        if not table:
            logger.warning(
                f"{self.__class__.__name__}: listingTable not found in panel content for year '{year_context}'."
            )
    ...
    def _extract_links_from_cells(
        self, cells: list, all_links_in_row: list, meeting_data: dict
    ) -> None:
        """
        ...
        for link_tag in all_links_in_row:
            ...
            if (
                "viewevent.php" in href.lower()
                or "mediaplayer.php" in href.lower()
                or any(
        ...
        for link_tag in all_links_in_row:
            ...
            link_text = link_tag.get_text(strip=True).lower()
            if (
                ("agenda" in link_text or "agendaviewer.php" in href.lower())
                and not meeting_data.get("packet_url") == href
                and not any(keyword in link_text for keyword in ["packet", "minutes"])
            ):  # Avoid misidentifying packet/minutes as agenda
        ...
        if not meeting_data.get("agenda_url") and len(cells) > 2:
            ...
            if (
                agenda_cell_link
                and agenda_cell_link["href"] != meeting_data.get("packet_url")
                and agenda_cell_link["href"] != meeting_data.get("minutes_url")
            ):
        ...
        if not meeting_data.get("minutes_url") and len(cells) > 3:
            ...
            if (
                minutes_cell_link
                and minutes_cell_link["href"] != meeting_data.get("agenda_url")
                and minutes_cell_link["href"] != meeting_data.get("packet_url")
            ):
        ...
        if meeting_data.get("agenda_url") and meeting_data.get(
            ...
            is_minutes_specific = (
                "minutesviewer.php" in (meeting_data.get("minutes_url", "")).lower()
            )
            is_agenda_specific = (
                "agendaviewer.php" in (meeting_data.get("agenda_url", "")).lower()
            )
            ...
            if is_agenda_specific and not is_minutes_specific:
                ...
                meeting_data["agenda_url"] = (
                    None  # Minutes is specific, agenda was likely the duplicate.
                )
            else:  # Both generic or both specific (unlikely for different types)
                meeting_data["minutes_url"] = (
                    None  # Default to clearing minutes if ambiguous and identical.
                )
