...
class GranicusType4Scraper(GranicusBaseScraper):
    ...
    def _extract_meeting_details_internal(
        ...
        main_tabbed_panels_content_group = soup.find(
            "div", class_="TabbedPanelsContentGroup"
        )

        if not main_tabbed_panels_content_group:
            logger.info(
                f"{self.__class__.__name__}: No 'TabbedPanelsContentGroup' (for years) found. This might not be a Type 4 structure or it's a simpler page."
            )
            # Fallback: Try to parse current view if no year tabs found.
            logger.info(
                f"{self.__class__.__name__}: Attempting to parse current page view for panel '{panel_name}'."
            )
            return self._extract_from_single_view_or_panel(
                soup, panel_name, "DefaultYear"
            )

        # Get all year content divs from the main TabbedPanelsContentGroup
        year_content_divs = main_tabbed_panels_content_group.find_all(
            "div", class_="TabbedPanelsContent", recursive=False
        )

        if not year_content_divs:
            logger.warning(
                f"{self.__class__.__name__}: 'TabbedPanelsContentGroup' found, but no 'TabbedPanelsContent' (year content) divs within it for panel '{panel_name}'."
            )
            return []

        # Try to get year names from corresponding tabs for context
        year_tabs_ul = soup.find(
            "ul", class_="TabbedPanelsTabGroup"
        )  # Assuming it's a sibling or related to ContentGroup
        year_tab_texts = ["UnknownYear"] * len(year_content_divs)
        if year_tabs_ul:
            year_tabs_li = year_tabs_ul.find_all(
            ...
            if len(year_tabs_li) == len(year_content_divs):
                ...
                logger.warning(
                    f"{self.__class__.__name__}: Mismatch in year tabs ({len(year_tabs_li)}) and year content divs ({len(year_content_divs)}). Using 'UnknownYear'."
                )
        ...
        for i, year_content_div in enumerate(year_content_divs):
            ...
            logger.info(
                f"{self.__class__.__name__}: Processing year content for '{year_context}', looking for panel '{panel_name}'."
            )
            ...
            meetings_from_year_panel = self._extract_from_single_view_or_panel(
                year_content_div, panel_name, year_context
            )
            if (
                meetings_from_year_panel
            ):  # If it returns a list (even empty), it means the panel was processed.
            ...
            # If _extract_from_single_view_or_panel returns None, it means panel_name was not found in that year_content_div
                ...
                meetings_from_year_panel is None
            ):  # Explicitly check for None if it signifies panel not found
                logger.debug(
                    f"{self.__class__.__name__}: Panel '{panel_name}' not found or no relevant content in year '{year_context}'."
                )

        if (
            not panel_ever_found and year_content_divs
        ):  # Only log if we actually processed year tabs
            logger.warning(
                f"{self.__class__.__name__}: Panel '{panel_name}' was not found in any year's content after checking all year tabs."
            )
            ...
            for yc_div in year_content_divs:  # Check within each year's parsed content
                collapsible_panels_in_year = yc_div.find_all(
                    "div", class_="CollapsiblePanel"
                )
                for cp_p in collapsible_panels_in_year:
                    h_div = cp_p.find(
                        ...
                        class_=[
                            "CollapsiblePanelTab",
                            "CollapsiblePanelTabNotSelected",
                        ],
                    )
                    if h_div:
                        txt_cont = h_div.find(["a", "h3", "span", "div"])
                        available_panels_overall.add(
                            txt_cont.get_text(strip=True)
                            if txt_cont
                            else h_div.get_text(strip=True)
                        )
            if available_panels_overall:
                logger.info(
                    f"{self.__class__.__name__}: Available panels found across years: {list(available_panels_overall)}"
                )

        logger.info(
            f"{self.__class__.__name__}: Extracted a total of {len(meetings)} meetings for panel '{panel_name}' across all available years."
        )
        return meetings
    ...
    def _extract_from_single_view_or_panel(
        ...
        if not collapsible_panels:
            logger.debug(
                f"{self.__class__.__name__}: No 'CollapsiblePanel' divs found in the current section for panel '{panel_name}' (Year: {year_context})."
            )
        ...
        for cp_panel in collapsible_panels:
            header_div = cp_panel.find(
                ["div", "h2", "h3"],
                class_=["CollapsiblePanelTab", "CollapsiblePanelTabNotSelected"],
            )
            if not header_div:
                continue

            text_container = header_div.find(["a", "h3", "span", "div"])
            current_panel_name_text = (
                text_container.get_text(strip=True)
                if text_container
                else header_div.get_text(strip=True)
            )

            if current_panel_name_text == panel_name:
                panel_found_in_section = True
                target_panel_content_div = cp_panel.find(
                    "div", class_="CollapsiblePanelContent"
                )
                if not target_panel_content_div:
                    logger.warning(
                        f"{self.__class__.__name__}: Panel '{panel_name}' (Year: {year_context}) header found, but 'CollapsiblePanelContent' is missing."
                    )
                break

        if not panel_found_in_section:
            logger.debug(
                f"{self.__class__.__name__}: Panel '{panel_name}' not found in this specific section (Year: {year_context})."
            )
            ...
            for cp_p_debug in collapsible_panels:  # Iterate again just for logging
                h_div_debug = cp_p_debug.find(
                    ["div", "h2", "h3"],
                    class_=["CollapsiblePanelTab", "CollapsiblePanelTabNotSelected"],
                )
                if h_div_debug:
                    txt_c_debug = h_div_debug.find(["a", "h3", "span", "div"])
                    available_panels_in_section.append(
                        txt_c_debug.get_text(strip=True)
                        if txt_c_debug
                        else h_div_debug.get_text(strip=True)
                    )
            if available_panels_in_section:
                logger.info(
                    f"{self.__class__.__name__}: Available panels in section (Year: {year_context}): {available_panels_in_section}"
                )
        ...
        meeting_list_container = target_panel_content_div.find(
            ["ol", "ul"], class_="responsive-table"
        )
        if not meeting_list_container:
            responsive_div_wrapper = target_panel_content_div.find(
                "div", class_="responsive-table"
            )
        ...
        if not meeting_list_container:
            logger.warning(
                f"{self.__class__.__name__}: Panel '{panel_name}' (Year: {year_context}) content found, but no 'responsive-table' <ol> or <ul> list inside."
            )
            return []  # Structure for meetings not found

        # If found the list, extract meetings from its items
        meetings.extend(
            self._extract_meetings_from_list_items(meeting_list_container, year_context)
        )
        return meetings

    def _extract_meetings_from_list_items(
        self, meeting_list_element: BeautifulSoup, year_context: str
    ) -> list[dict]:
        """
        ...
        if (
            not meeting_list_items
        ):  # Fallback if no "table-row", try direct <li> children
        ...
        for item_idx, list_item in enumerate(meeting_list_items):
            ...
            # Name
            name_div = list_item.find(
                "div", class_="archive-name"
            )  # Common class for name/title
            if name_div:
                ...
                name_text_candidate = list_item.find(
                    ["span", "div"], recursive=False
                )  # First direct span/div
                if name_text_candidate:
                    meeting_data["name"] = name_text_candidate.get_text(
                        strip=True
                    ).replace("\xa0", "")

            # Date
            date_div = list_item.find(
                "div", class_="archive-date"
            )  # Common class for date
            if date_div:
            ...
            if not meeting_data.get("name") or not meeting_data.get("date"):
                ...
                if not meeting_data.get("name"):
                    name_labelled_div = list_item.find(
                        "div", attrs={"data-label": re.compile(r"Name|Event", re.I)}
                    )
                    if name_labelled_div:
                        meeting_data["name"] = name_labelled_div.get_text(
                            strip=True
                        ).replace("\xa0", "")
                if not meeting_data.get("date"):
                    date_labelled_div = list_item.find(
                        "div", attrs={"data-label": "Date"}
                    )
                    if date_labelled_div:
                        meeting_data["date"] = date_labelled_div.get_text(strip=True)

                if not meeting_data.get("name") or not meeting_data.get("date"):
                    logger.debug(
                        f"Skipping list item {item_idx+1} due to missing name or date: {list_item.get_text(strip=True)[:100]}"
                    )
                    continue
                ...
                if section_div:
                    ...
                    if link_tag:
                        ...
                        if not meeting_data.get("meeting_id_source"):
                            id_match = re.search(
                                r"[?&](?:clip_id|event_id|meeting_id)=(\d+)",
                                link_tag["href"],
                                re.IGNORECASE,
                            )
                            if id_match:
                                meeting_data["meeting_id_source"] = id_match.group(1)
            ...
            if not meeting_data.get("agenda_url"):
                for a_tag in all_links_in_item:
                    if (
                        re.search(r"Agenda", a_tag.get_text(strip=True), re.I)
                        and "packet" not in a_tag.get_text(strip=True).lower()
                    ):
                        meeting_data["agenda_url"] = a_tag["href"]
                        break
            if not meeting_data.get("packet_url"):
                for a_tag in all_links_in_item:
                    if re.search(
                        r"Packet|Agenda Packet", a_tag.get_text(strip=True), re.I
                    ):
                        meeting_data["packet_url"] = a_tag["href"]
                        break
            if not meeting_data.get("minutes_url"):
                for a_tag in all_links_in_item:
                    if re.search(r"Minutes", a_tag.get_text(strip=True), re.I):
                        meeting_data["minutes_url"] = a_tag["href"]
                        break
            if not meeting_data.get("video_url"):
                for a_tag in all_links_in_item:
                    if (
                        re.search(
                            r"Video|Watch|Media", a_tag.get_text(strip=True), re.I
                        )
                        or "ViewEvent.php" in a_tag["href"]
                        or "MediaPlayer.php" in a_tag["href"]
                    ):
                        meeting_data["video_url"] = a_tag["href"]
                        break
            ...
            if not meeting_data.get("time") and meeting_data.get("date"):
                time_match_item = re.search(
                    r"(\d{1,2}:\d{2}\s*(?:AM|PM|am|pm)?)", meeting_data["date"]
                )
                if time_match_item:
            ...
            if meeting_data.get("name") and meeting_data.get("date"):
                ...
                logger.debug(
                    f"Final check failed for list item {item_idx+1}, missing name or date after all attempts."
                )
