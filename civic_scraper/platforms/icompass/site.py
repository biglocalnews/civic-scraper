...
class ICompassSite(base.Site):
        ...
        for a in soup.select("a.meeting-type-item-title"):
            ...
            if href:
                full_url = (
                    ...
                    else driver.current_url.split("/Portal")[0] + href
                )
    ...
    def _extract_meeting_links(self, html, base_url):
        ...
        for btn in soup.find_all(
            "button", class_=re.compile(r"meeting-list-item-button")
        ):
            btn_id = btn.get("id", "")
            ...
            meeting_name = (
                name_divs[1].get_text(strip=True) if len(name_divs) > 1 else None
            )
            if meeting_id:
                info_url = urljoin(
                    base_url, f"/Portal/MeetingInformation.aspx?Id={meeting_id}"
                )
                meetings.append(
                    {
    ...
    def _get_meetings(self, committee_url=None):
        ...
        if resp.status_code != 200:
            print(
                f"[WARNING] Failed to fetch committee page {committee_url}: HTTP {resp.status_code}"
            )
            return []
    ...
    def scrape(self, download=False, start_date=None, end_date=None):
        ...
        if resp.status_code != 200:
            print(
                f"[ERROR] Failed to fetch main page {self.url}: HTTP {resp.status_code}"
            )
        ...
        for a in soup.select("a.meeting-type-item-title"):
            ...
            if href:
                full_url = (
                    href
                    if href.startswith("http")
                    else self.url.split("/Portal")[0] + href
                )
                committee_links.append((name, full_url))
        if self.committee_names:
            committee_links = [
                cl for cl in committee_links if cl[0] in self.committee_names
            ]
            ...
            for m in meetings:
                ...
                try:
                    ...
                    if resp_page.status_code == 200:
                        ...
                        title_tag = soup_page.find(
                            "div", id="ctl00_MainContent_MeetingTitle"
                        )
